...
describe("Tracing Controller", async () => {
    describe("GET /api/getconfirmedcontacts/:mrn", async () => {
        context("Wanda Hastings (1def220e-b4e8-11ec-a016-ac87a3187c5f)", async () => {
            const patientMrn = "1def220e-b4e8-11ec-a016-ac87a3187c5f";
            let res: Response;
            before(async () => {
                res = await chai.request(server).get(`/api/getconfirmedcontacts/${patientMrn}`);
                expect(res).to.exist;
            ...
            it("Should return a contact list", async () => {
                ...
                expect(res.body.contactlist).to.be.an("array").that.is.not.empty;
            });
        ...
        context("Henrietta Cook (1def2240-b4e8-11ec-a016-ac87a3187c5f)", async () => {
    ...
    describe("GET /api/getpossiblecontacts/:mrn", async () => {
        context("Wanda Hastings (1def220e-b4e8-11ec-a016-ac87a3187c5f)", async () => {
            ...
            let res: Response;
            before(async () => {
                res = await chai.request(server).get(`/api/getpossiblecontacts/${patientMrn}`);
                expect(res).to.exist;
                expect(res).to.have.status(200);
                expect(res.body).to.exist;
            });
            ...
            context("Event 1def1d22-b4e8-11ec-a016-ac87a3187c5f", async () => {
                const eventId = "1def1d22-b4e8-11ec-a016-ac87a3187c5f";
                let eventContactList: any;
                it("Should return event contact list", async () => {
                    eventContactList = _.find(res.body.contactlist, { [eventId]: [] });
                    expect(eventContactList).to.exist;
                    expect(eventContactList[eventId]).to.exist;
                    expect(eventContactList[eventId]).to.have.length.above(0);
                });
                it("Should find every other patient who attended event", async () => {
                    expect(eventContactList[eventId]).to.have.members([
                        '1def2268-b4e8-11ec-a016-ac87a3187c5f'
                    ])
                });
            });
            context("Event 1def1d9a-b4e8-11ec-a016-ac87a3187c5f", async () => {
        ...
        context("Henrietta Cook (1def2240-b4e8-11ec-a016-ac87a3187c5f)", async () => {
            ...
            context("Event 1def2006-b4e8-11ec-a016-ac87a3187c5f", async () => {
            ...
            context("Event 1def202e-b4e8-11ec-a016-ac87a3187c5f", async () => {
